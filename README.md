...
```
SetThrowError(bool flag); // エラー出力切り替え設定
```
...
```
SetCreateLogFile(bool flag); // エラーログファイルを作成設定
```
...
```
SetCreateSAELibFile(bool flag, const std::string& Path = ""); // SAELibフォルダを作成設定
```
...
```
SetSAELibFilePath(const std::string& Path = ""); // SAELibフォルダのパス設定
```
